...
| `Ctrl`+`Alt`+`0` ... `Ctrl`+`Alt`+`8` | Select format 11 to 18. |
...
| 𝓜𝓪𝓽𝓱𝓮𝓶𝓪𝓽𝓲𝓬𝓪𝓵 𝓑𝓸𝓵𝓭 𝓢𝓬𝓻𝓲𝓹𝓽 | `mathBoldScript` | `Ctrl`+`7` |
| 𝖬𝖺𝗍𝗁𝖾𝗆𝖺𝗍𝗂𝖼𝖺𝗅 𝖲𝖺𝗇𝗌-𝖲𝖾𝗋𝗂𝖿[^1] | `mathSans` | `Ctrl`+`8` |
| 𝗠𝗮𝘁𝗵𝗲𝗺𝗮𝘁𝗶𝗰𝗮𝗹 𝗦𝗮𝗻𝘀-𝗦𝗲𝗿𝗶𝗳 𝗕𝗼𝗹𝗱[^1] | `mathSansBold` | `Ctrl`+`9` |
| 𝘔𝘢𝘵𝘩𝘦𝘮𝘢𝘵𝘪𝘤𝘢𝘭 𝘚𝘢𝘯𝘴-𝘚𝘦𝘳𝘪𝘧 𝘐𝘵𝘢𝘭𝘪𝘤[^3] | `mathSansItalic` | `Ctrl`+`Alt`+`0` |
| 𝙈𝙖𝙩𝙝𝙚𝙢𝙖𝙩𝙞𝙘𝙖𝙡 𝙎𝙖𝙣𝙨-𝙎𝙚𝙧𝙞𝙛 𝘽𝙤𝙡𝙙 𝙄𝙩𝙖𝙡𝙞𝙘[^4] | `mathSansBoldItalic` | `Ctrl`+`Alt`+`1` |
| 𝙼𝚊𝚝𝚑𝚎𝚖𝚊𝚝𝚒𝚌𝚊𝚕 𝙼𝚘𝚗𝚘𝚜𝚙𝚊𝚌𝚎[^1] | `mathMono` | `Ctrl`+`Alt`+`2` |
| Ｆｕｌｌｗｉｄｔｈ[^1] | `fullwidth` | `Ctrl`+`Alt`+`3` |
| Ⓒⓘⓡⓒⓛⓔⓓ[^1] | `circled` | `Ctrl`+`Alt`+`4` |
| 🅝🅔🅖🅐🅣🅘🅥🅔 🅒🅘🅡🅒🅛🅔🅓[^1][^5] | `negativeCircled` | `Ctrl`+`Alt`+`5` |
| 🅂🅀🅄🄰🅁🄴🄳[^5] | `squared` | `Ctrl`+`Alt`+`6` |
| 🅽🅴🅶🅰🆃🅸🆅🅴 🆂🆀🆄🅰🆁🅴🅳[^4] | `negativeSquared` | `Ctrl`+`Alt`+`7` |
| 🄟⒜⒭⒠⒩⒯⒣⒠⒮⒤⒵⒠⒟[^6] | `parenthesized` | `Ctrl`+`Alt`+`8` |
