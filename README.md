...
| 𝑴𝒂𝒕𝒉𝒆𝒎𝒂𝒕𝒊𝒄𝒂𝒍 𝑩𝒐𝒍𝒅 𝑰𝒕𝒂𝒍𝒊𝒄[^2] | `mathBoldItalic` | `Ctrl`+`3` |
| 𝕸𝖆𝖙𝖍𝖊𝖒𝖆𝖙𝖎𝖈𝖆𝖑 𝕭𝖔𝖑𝖉 𝕱𝖗𝖆𝖐𝖙𝖚𝖗[^2] | `mathBoldFrakt` | `Ctrl`+`4` |
...
| 𝘔𝘢𝘵𝘩𝘦𝘮𝘢𝘵𝘪𝘤𝘢𝘭 𝘚𝘢𝘯𝘴-𝘚𝘦𝘳𝘪𝘧 𝘐𝘵𝘢𝘭𝘪𝘤[^3] | `mathSansItalic` | `Ctrl`+`8` |
| 𝙈𝙖𝙩𝙝𝙚𝙢𝙖𝙩𝙞𝙘𝙖𝙡 𝙎𝙖𝙣𝙨-𝙎𝙚𝙧𝙞𝙛 𝘽𝙤𝙡𝙙 𝙄𝙩𝙖𝙡𝙞𝙘[^4] | `mathSansBoldItalic` | `Ctrl`+`9` |
| 𝙼𝚊𝚝𝚑𝚎𝚖𝚊𝚝𝚒𝚌𝚊𝚕 𝙼𝚘𝚗𝚘𝚜𝚙𝚊𝚌𝚎[^1] | `mathMono` | `Ctrl`+`Alt`+`0` |
| Ｆｕｌｌｗｉｄｔｈ[^1] | `fullwidth` | `Ctrl`+`Alt`+`1` |
| Ⓒⓘⓡⓒⓛⓔⓓ[^1] | `circled` | `Ctrl`+`Alt`+`2` |
| 🅝🅔🅖🅐🅣🅘🅥🅔 🅒🅘🅡🅒🅛🅔🅓[^1] [^5] | `negativeCircled` | `Ctrl`+`Alt`+`3` |
| 🅂🅀🅄🄰🅁🄴🄳[^5] | `squared` | `Ctrl`+`Alt`+`4` |
| 🅽🅴🅶🅰🆃🅸🆅🅴 🆂🆀🆄🅰🆁🅴🅳[^4] | `negativeSquared` | `Ctrl`+`Alt`+`5` |
| 🄟⒜⒭⒠⒩⒯⒣⒠⒮⒤⒵⒠⒟[^6] | `parenthesized` | `Ctrl`+`Alt`+`6` |
